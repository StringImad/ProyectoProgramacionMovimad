...
public class BD_ControladorViajeCRUD {
	...
	public void createViaje(Viaje viaj) {
		this.em = entityManagerFactory.createEntityManager();
		// En este caso es necesario iniciar una transacción en la base de datos
		// porque vamos a persistir información en la misma
		this.em.getTransaction().begin();
...
//	public List<Viaje> findByNumSeg(String numSeg) {
//		this.em = entityManagerFactory.createEntityManager();
//		this.consulta = em.createNativeQuery("Select * from viaje where puntorecogida=?", Viaje.class);
//		this.consulta.setParameter(1, numSeg);
//		List<Viaje> listaUsuarios = (List<Viaje>) consulta.getResultList();
//		this.em.close();
